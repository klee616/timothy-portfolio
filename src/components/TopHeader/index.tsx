...
<div className='h-screen w-[70vw] z-[50]  flex flex-col justify-start align-center items-center gap-4 fixed top-0 left-0 bg-[#3b82f6]'>
      ...
      <header className={` z-[100] w-full md:fixed md:flex md:justify-between md:top-4 md:bg-transparent md:pl-10 md:pr-10  z-[100]  ${showHamburgerMenu} top-0 right-0`}>
        <div className='w-full md:max-w-7xl md:w-full flex justify-between items-center md:shadow-xl pl-5 pr-5  md:pl-10 md:pr-10 md:border-white md:border-4 md:rounded-full md:bg-[var(--second-color)]'>
          ...
          <div className={`block md:hidden flex flex-col-reverse divide-y-4 divide-y-reverse divide-[var(--second-color)] w-[60px] h-[60px] justify-around`} onClick={isShowMenu}>
            <div></div>
            <div></div>
            <div></div>
            <div></div>
